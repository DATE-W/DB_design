  ...
  <div class="total">
    ...
    <div class="common-layout">
      <el-container>
          ...
          <div class="containerLeague">
            <el-row>
              ...
              <el-col :span="18" class="leftLeague">
                <p class="titleLeagueLeft">中超动态</p>
                <div class="line" style="width: 40vw;height: 0.2px;top:-12vh;left:9%;"></div>
                <div v-for="item in ChinaNews" :key="item.id" class="itemLeague">
              ...
              <!-- 右侧球队一览+排行榜+最新新闻 -->
              <el-col :span="6" class="rightLeague">
                <div class="floating-block" :class="{ 'fixedLeague': isFixed }">
                  <p class="titleLeagueLeft" style="font-size: 25px;left:3%;">中超球队一览</p>
                  <div class="line" style="width: 22vw;height: 0.2px;top:-12.5vh;left:3%;"></div>
                  <div class="Teamcontainer">
                    <el-row :gutter="10" v-for="(row, index) in 5" :key="index">
                      <el-col :span="6" v-for="(image, index) in ChinaTeam.slice((row - 1) * 4, row * 4)" :key="index">
                        <img :src="image" class="imgTeam" alt="image" />
                        <a class="textTeam">队伍名称</a>
                      </el-col>
                    </el-row>
                  </div>
                </div>
                <div class="remaining-content">
                  <p class="titleLeagueLeft" style="font-size: 25px;left:-15%;">积分榜</p>
                  <div class="line" style="width: 22vw;height: 0.2px;top:-13vh;left:-15%;"></div>
                  <el-row :gutter="10" v-for="(row, index) in 20" :key="index">
                    <el-col :span="6">
        ...
        <!-- 英超 -->
        <el-container style="top:22vh;position: absolute;" v-if="activeTab === 'England'">
          英超
          <!-- 前后端交互测试 -->
          <div>
            <button @click="getData(2, '中超', '普通', items)">获取数据</button>

            <p1>访问成功？{{ ok }}</p1>

            <!-- <ul>
              <li v-for="(item, index) in items" :key="index">
                <h3>{{ item.newsBody.title }}</h3>
                <p>{{ item.newsBody.summary }}</p>
                <img :src="getimgurl(item.pictureRoutes[0])" alt="carousel image" class="imgANO">
              </li>
            </ul> -->
          </div>
          <!-- 前后端交互测试结束 -->
        ...
        <!-- 西甲 -->
        <el-container style="top:22vh;position: absolute;" v-if="activeTab === 'Spain'">
          西甲
          <img referrerPolicy='no-referrer' :src="imageUrl"  alt="carousel image" style="width:500px">
        </el-container>
        <!-- 德甲 -->
...
import imgForTest from '../assets/img/lllogo.png';
...
export default {
  ...
  data() {
    return {
      imageUrl:'https://i0.hdslb.com/bfs/archive/9b45084ba55c0b9704fe58e178d9633f6c8fbe89.jpg@672w_378h_1c_!web-search-common-cover',
      ok: '',
      items: [1, 2, 3], // 存储返回的数据的数组
      ...
      ChinaTeam: [   //中超队徽
        imgForTest, imgForTest, imgForTest, imgForTest, imgForTest, imgForTest, imgForTest, imgForTest, imgForTest, imgForTest,
        imgForTest, imgForTest, imgForTest, imgForTest, imgForTest, imgForTest, imgForTest, imgForTest, imgForTest, imgForTest,
  ...
  methods: {
    ...
    handleScroll() {
      ...
      const threshold = 200; // 右侧部分滑动到多少位置时触发悬浮效果
