<!-- 2154314_郑楷_赛事详情页 2023.08.11 14:00 v1.1.0 
...
<div class="topTextBox" style="left: 35rem;top:5rem;">
  <p class="textDate">{{ this.dateChoice }}&nbsp&nbsp{{ this.dateTime }}</p>
  <p class="textStatus">{{ getMatchStatus(this.status) }}</p>
  <p class="textLeague">{{ getLeagueInfo(this.leagueInfoNum) }}</p>
</div>

<div class="topTextBoxTRA" style="left: 13rem;top:5rem;width:20rem" @click="toTeams(this.homeTeamName)">
  <p class="textTeamName" style="left:1rem">{{ this.homeTeamName }}</p>
  <img class="picTeam" :src=this.homeLogo style="left:2rem">
</div>

<div class="topTextBoxTRA" style="left: 64rem;top:5rem;width:20rem" @click="toTeams(this.guestTeamName)">
  <p class="textTeamName" style="right:1rem">{{ this.guestTeamName }}</p>
  <img class="picTeam" :src=this.guestLogo style="right:2rem">
</div>

<div class="scoreBox" style="left: 31rem;top:8rem">
  <p class="textScore">{{ this.homeScore }}</p>
</div>

<div class="scoreBox" style="left: 61rem;top:8rem">
  <p class="textScore">{{ this.guestScore }}</p>
</div>
...
<div class="recentGameBoxLeft" v-for="(homeRecentGame,index) in homeRecentGames" :key="homeRecentGame.gameUid"
:style="{ top: `${index * 7+22}rem` }" @click="toMatchDetail(homeRecentGame.gameUid,1,homeRecentGame.gameDate)" style="left:13rem;">
  <p class="textMatchDate">{{ homeRecentGame.gameDate }}</p>
  <p class="textMatchScore" style="left:5.5rem">{{ homeRecentGame.homeScore }}&nbsp:&nbsp{{ homeRecentGame.opponentScore }}</p>
  <p class="textMatchOpp" style="right:0.5rem">{{ homeRecentGame.opponentName }}</p>
</div>

<div class="recentGameBoxLeft" v-for="(guestRecentGame,index) in guestRecentGames" :key="guestRecentGame.gameUid"
:style="{ top: `${index * 7+22}rem` }" @click="toMatchDetail(guestRecentGame.gameUid,1,guestRecentGame.gameDate)" style="left:64rem;">
  <p class="textMatchDate" style="right:0">{{ guestRecentGame.gameDate }}</p>
  <p class="textMatchScore" style="right:5.5rem">{{ guestRecentGame.opponentScore }}&nbsp:&nbsp{{ guestRecentGame.homeScore }}</p>
